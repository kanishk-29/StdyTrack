...
| FR-13.7 | A tap on the mascot avatar (a click that is not a drag) shall toggle an "Ask Rei" chat panel where the user can type a freeform question about their own tracker data (e.g. lecture counts per subject, subject status, what to study next). The system shall answer from the tracker's actual data: when the optional AI module is online, via Gemini (ReiAI.answerChat, restricted to facts supplied in the prompt); otherwise, or when Gemini is unavailable, via a local factual answerer in js/mascot.js (lecture counts, subject status/details, recommend-next), and then the legacy conversational intents for open-ended questions. The panel must: render user and Rei messages safely (text-only), show a typing state while an answer is pending, never block the timer flow, close via its ✕ button or a second tap, and be hidden whenever the mascot is minimized. Fallback behaviour must always return a non-empty, data-grounded or politely-declined answer. | Low |
